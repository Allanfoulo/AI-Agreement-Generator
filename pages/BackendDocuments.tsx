...
export function BackendDocuments({ clients }: { clients: Client[] }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="bg-white rounded-xl p-6 border"><h2 className="text-xl">Documents</h2>{documents.map(d => <article key={d._id} className="border-b py-4"><h3>{d.number ?? 'Draft'} · {d.content.title}</h3><p>{d.status} · {d.content.currency} {(d.totalMinor / 100).toFixed(2)}</p><div className="flex gap-4 flex-wrap">
    ...
    <section className="bg-white rounded-xl p-6 border"><h2 className="text-xl">PDF downloads</h2>{jobs.map(j => <div key={j._id} className="py-2">{j.status} · attempts {j.attempts} {j.checksum && <span className="text-xs text-gray-500">· checksum {j.checksum.slice(0, 12)}…</span>} {j.url && <a className="underline" href={j.url} target="_blank" rel="noreferrer">Download PDF</a>}{j.status === 'failed' && <button onClick={() => run(() => retry({ jobId: j._id }))}>Retry</button>}</div>)}</section>
