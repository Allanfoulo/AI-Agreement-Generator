...
export function LegacyImport() {
  ...
  return <section className="studio-page import-page bg-white p-6 rounded-xl"><span className="section-kicker">03 / BRING IT WITH YOU</span><h1 className="text-2xl mb-4">Import existing browser data</h1><p>Preview your original browser records before bringing them into the shared workspace.</p><div className="flex flex-wrap gap-3 mt-5"><button disabled={busy} onClick={preview} className="button-primary">Preview import</button>{payload && <button disabled={busy} onClick={commit} className="button-quiet">Import records</button>}</div><p role="status" className="studio-feedback-inline">{message}</p></section>;
}
